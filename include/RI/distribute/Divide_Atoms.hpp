...
namespace Divide_Atoms
{
	...
	*/
		...
		const std::array<Tcell,Ndim> &period)
	{
		...
		const std::vector<std::array<Tcell, Ndim>> cells_origin = traversal_period(period);
		const std::vector<std::array<Tcell, Ndim>> cells = Global_Func::mod_period(cells_origin, period);
		...
		for(const TA &atom : atoms_divide)
			for(const std::array<Tcell,Ndim> &cell : cells)
		...
		const std::array<Tcell,Ndim> &period)
	{
		...
		std::vector<std::pair<TA,std::array<Tcell,Ndim>>> atoms_periods_divide;
